...
import { HomeworkInfo } from "./dialogs/homework-info";
...
export function HomeworkTable({
  ...
  const [selectedHomework, setSelectedHomework] =
    React.useState<HomeworkRow | null>(null);
  const [infoDialogOpen, setInfoDialogOpen] = React.useState(false);

  const completedParam =
    statusFilter === "all" ? undefined : statusFilter === "completed";

  const { query, updateMutation, deleteMutation } = useHomeworkData(
    completedParam,
    subjectId,
  );
  const { subjects } = useSubjects();
  const data = React.useMemo(
    () => (query.data ?? []) as HomeworkRow[],
    [query.data],
  );
  ...
  const handleRowClick = React.useCallback(
    (homework: HomeworkRow, event: React.MouseEvent) => {
        ...
        : ([
            {
              ...
              sortingFn: (rowA, rowB) => {
                const subjectA =
                  subjectMap.get(rowA.original.subjectId)?.name || "";
                const subjectB =
                  subjectMap.get(rowB.original.subjectId)?.name || "";
