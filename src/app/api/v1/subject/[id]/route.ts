...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const id = params.id;
  const row = await db.select().from(subjects).where(eq(subjects.id, id)).then(r => r[0]);
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const updated = await db.select().from(subjects).where(eq(subjects.id, id)).then(r => r[0]);
